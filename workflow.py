...
def classifyPose(landmarks, output_image, display=False,count=0, pose=0):
    '''
    ...
    cv2.putText(output_image, f'{func_poses[pose]}', (10, 60),cv2.FONT_HERSHEY_PLAIN, 2, color, 2)
...
while camera_video.isOpened():
    ...
    example_img = cv2.imread(f'poses/{poses[pose_count]}.jpg')
    size = 200
    ...
    if landmarks:
        ...
        frame, feedback = classifyPose(landmarks, frame, display=False,count=count,pose=pose_count)
        if feedback == 'good':
            if count>50:
                pose_count+=1
                count = 0
